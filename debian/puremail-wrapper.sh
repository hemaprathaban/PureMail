...
#
# Purpose:
#   This is a wrapper script for starting the puremail binary with taking
...
#   Mozilla default for the puremail binary is '${HOME}/.puremail'.
#   The script will looking for the old profile folder and will symlink the
#   folder '${HOME}/.puremail' to the profile folder '${HOME}/.icedove'.
#
# Copyright:
...
TB_HELPER="/usr/lib/puremail/puremail-wrapper-helper.sh"
...
# profile(s) folder ~/.puremail will be also reworked. That's not the
# case if the user is starting this wrapper for a first time and only a
# folder ~/.puremail is existing!
if [ "${FIXMIME}" = "1" ]; then
...
#############################################################################
#                  User puremail Profile Adoptions                       #
#                                                                           #
# The users Icedove/puremail profile(s) doesn't need to be modified in a #
# different and complicated way. We simply need to ensure that the          #
# puremail binary is finding the existing profiles in the folder         #
# $(HOME)/.puremail folder or a valid symlink pointing to the profiles.  #
#                                                                           #
# To "migrate" an old existing Icedove profile we can simply do a symlink   #
# from $(HOME)/.puremail --> $(HOME)/.icedove .                          #
#                                                                           #
...
if { [ -d "${ID_PROFILE_FOLDER}" ] || [ -L "${ID_PROFILE_FOLDER}" ]; } && \
   { [ ! -d "${TB_PROFILE_FOLDER}" ] && [ ! -L "${TB_PROFILE_FOLDER}" ]; }; then
    ...
    output_debug "Start puremail profile adoptions, please be patient!"
    ...
    # we are finished
    output_info "puremail Profile adoptions done."
    do_collect_backup_files
fi

# We found both profile folder, and .puremail is a symlink,
# we need to check if .puremail is symlinked to .icedove
if { [ -d "${ID_PROFILE_FOLDER}" ] && [ -L "${TB_PROFILE_FOLDER}" ]; } && \
   [ "$(readlink -e "${TB_PROFILE_FOLDER}")" = "${ID_PROFILE_FOLDER}" ];then
    ...
    # before we switched back to puremail.
...
# ... or the opposite if .icedove is symlinked to .puremail
...
# has probably an old or otherwise used puremail installation. Which one
# is the correct one to use?
    ...
    output_debug "Please investigate by yourself! Maybe you will find additional information in '/usr/share/doc/puremail/README.Debian.gz'."
    logger -i -p warning -s "$0: [profile migration] Couldn't migrate Icedove into puremail profile due existing or symlinked folder '${TB_PROFILE_FOLDER}'!"
...
# If we are here we going simply further by starting puremail.
...
if [ "${DEBUG}" = "" ]; then
    ...
    if [ "${DEBUGGER}" = "1" ]; then
        ...
        if [ -f /usr/bin/gdb ]; then
            if dpkg-query -W -f='${Version}' puremail-dbg &>/dev/null ; then
                output_info "Starting puremail with GDB ..."
                LANG='' "${MOZ_LIBDIR}"/run-mozilla.sh -g "${MOZ_LIBDIR}"/"${MOZ_APP_NAME}" "${TB_ARGS[@]}"
            else
                output_info "No package 'puremail-dbg' installed! Please install first and restart."
                exit 1
            fi
        else
